...
export const createCardElement = (card, { selectable = false, selected = false } = {}) => {
  ...
  rank.className = "card-rank"; rank.textContent = card?.rank ?? "?";
  const suitSymbol = document.createElement("span");
  suitSymbol.className = "card-suit"; suitSymbol.textContent = SUITS[card?.suit] ?? "?";
  const suitName = document.createElement("span");
  suitName.className = "card-name"; suitName.textContent = card?.suit ?? "unknown";
